...
NotePlayer::NotePlayer(NotePlayer& note_player)
{
  note_player.audio_buffer = audio_buffer;
  note_player.pcm_wave_format = pcm_wave_format;
  note_player.waveout = waveout;
  note_player.mm_result = mm_result;
...
void NotePlayer::PlayMelody(std::string notes, std::string rythm, double tempo_bpm)
{
  ...
  for(auto i=0;i<splited_notes.size();++i)
  {
